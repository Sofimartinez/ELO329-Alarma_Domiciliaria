...
void DoorView::makeDoorView(){
    QGraphicsPolygonItem * origenPillar=new QGraphicsPolygonItem(this); //pilar bisagra
    QPolygonF p;
    ...
    doorPanel = new QGraphicsRectItem(10, 10, 160, 10, this); //dimensiones de la puerta
...
void DoorView::setOpen(){
    doorPanel->setRotation(90); //Rota en 90 grados para abrir la puerta
    magnet->setRotation(90); //Rota en 90 para cambiar la posición del iman junto con la puerta
}
void DoorView::setClose(){
    doorPanel->setRotation(0); //Posición inicial
    magnet->setRotation(0);
}
void DoorView::mousePressEvent(QGraphicsSceneMouseEvent * event){
    if (model!= NULL && event->button()==Qt::LeftButton) //solo si se presiona con el boton izquierdo
        model->changeState();
}
DoorView::~DoorView(){
    delete doorPanel;
